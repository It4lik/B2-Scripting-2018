...
def pullRepo(repo_url, local_repo_path):
    ...
    try:
        git_pull_command = "git --git-dir=" + local_repo_path + "/.git pull &> /dev/null"
        ...
        stderr.write((str(e)))
        stderr.write("\nCan't pull in " + local_repo_path + " from " + repo_url)
...
faulty_repo_owners = []
updated_repo_owners = []
nawak_repo_owners = [] # Will store the repos whose name or content are non as asked
...
with open(github_urls_file, "r") as file:
    ...
    for line in file.readlines():
        ...
        # If repo name isn't correctly formatted, just drop it :)
        repo_name = repo_git_url.split('/')[-1]
        if repo_name != "B2-Python":
            nawak_repo_owners.append(repo_owner_name)
            stderr.write("\nRepo " + repo_name + " of " + repo_owner_name + " doesn't follow the name rule.")
            continue


        # Test if the destination dir exists, if yes, pull the repo, otherwise, clone it.
        if path.isdir(local_repo_path):
            stdout.write("\nThe repo of " + repo_owner_name + " has already been cloned. Pulling it.")
...
stdout.write("\n  " + str(len(faulty_repo_owners)) + " were faulty"
             + "\n  " + str(len(successfully_cloned_repos_owners)) + " were cloned"
